\let\bf\bfseries
\let\it\itshape
\section{(7) (WIP)Транспортные сети. Задача о максимальном потоке. Разрез. Теорема о максимальном потоке и минимальном разрезе. Алгоритм Форда-Фалкерсона (\groth)}
...
\FFA($\langle G=\langle V,E\rangle,c,s,t\rangle$):\\
    \For{\textbf{each} $(u, v) \in E$}{
        $f(u, v)$ := 0\\
        $f(v, u)$ := 0\\
    }
    \While{$\exists\textrm{ путь }p\colon s\to t \textrm{ в } G_f$}{
        $c_f(p)$ := $\min\{c_f(u,v)|(u,v)\in p\}$\\
        \For{\textbf{each} $(u, v) \in p$}{
            $f(u,v)$ += $c_f(p)$\\
            $f(v,u)$ := $-f(u,v)$
        }
    }
