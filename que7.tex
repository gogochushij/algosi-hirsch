...
На практике, понятно, он возникает в основном только с целыми числами. Проблема этого алгортима в том, что не указано, как именно нужно искать увеличивающий путь. Если искать его неудачно, то алгоритм может и зависнуть.
...
В предположении, что числа рациональные (их можно свести к целым) и при использовании поиска в глубину, время его работы составляет $O(|E||f^*|)$, где $f^*$~--- максимальный поток.
...
ааа тут я ничего не понимаю
%Поиск в глубину работает за $O(|V|+|E|)$. Поэтому весь алгоритм выполняется за время $O(|E||f^*|)$.
...
\begin{example}
	...
	\end{tikzpicture}
	\end{center}
	Поиском в глубину находится путь $s\to a\to b\to t$, поэтому за одну итерациюю поток увеличивается всего лишь на единицу.
\end{example}
...
Понятно, что максимальному паросочетанию соотвествтует максимальный поток (поскольку иначе существует паросочетание $|M'|=|f'|>|f|=|M|)$.
